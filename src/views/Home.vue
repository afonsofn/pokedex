  ...
  <div class="home">
    <section class="poke-description">
      ...
      <div class="poke-info">
        ...
        <div class="info">
          weight: <p>{{ pokemonDetails && pokemonDetails.weight }}lb</p>
        </div>
        <div class="info">
          games: <p>{{ pokemonDetails && pokemonDetails.game_indices.map(version => version.version.name.toUpperCase()).join(', ') }}</p> 
        </div>
        <div class="info">
          locations: <p>{{ location && location.map(location_area => location_area.location_area.name.toUpperCase()).join(', ') }}</p>
        </div>
      </div>
    </section>
    <section class="poke-list">
      <template v-for="(pokemon, index) in allPokemons">
        <div v-bind:key="index" class="poke-item">
